...
export default function MedicalCentersClient() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.medicalCenters.list({ page, pageSize: 15, search: search || undefined }),
  });

  const publishMut = useMutation({
    mutationFn: ({ id, published }: any) => published ? api.medicalCenters.unpublish(id) : api.medicalCenters.publish(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['med-centers'] }); toast('success', 'تم تحديث حالة النشر'); },
    onError: (e: any) => toast('error', e.message),
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => api.medicalCenters.delete(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['med-centers'] }); setDeleteTarget(null); toast('success', 'تم الحذف'); },
    onError: (e: any) => toast('error', e.message),
  });

  const columns = [
    { key: 'name', header: 'الاسم', render: (r: any) => (
      <div><p className="font-medium">{r.name?.ar}</p><p className="text-xs text-gray-400">{r.name?.en}</p></div>
    )},
    { key: 'specialty', header: 'التخصص', render: (r: any) => r.specialty?.ar || '—' },
    { key: 'hospitals', header: 'المستشفيات', render: (r: any) => (
      <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{r.hospitals?.length ?? 0}</span>
    )},
  ...
  return (
    <div>
      <PageHeader title="المراكز الطبية" subtitle={`${data?.meta.total ?? 0} مركز`} action="إضافة مركز" onAction={() => { setEditing(null); setModalOpen(true); }} />
      <div className="bg-white rounded-2xl border border-gray-100 p-4">
        <div className="mb-4 max-w-xs"><SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="بحث في المراكز..." /></div>
