...
| 13 | ~~**Unzip the facility photographs**~~ | **Done 2026-07-30, this row was stale until 2026-08-02.** 58 photographs are filed across 13 domain folders — Inpatient Rooms 19, Outpatient 9, Operating Room 8, Radiology 7. `Laboratory` and `Physiotherapy` are still empty, and the original `.zip` is still sitting in `Services/Pics/` | §7 |
...
| **A domain folder with no photographs falls back silently** | Generation drops to AI_GENERATED with no reference and says nothing. **Measured 2026-08-02: 58 photographs are filed across 13 folders**, so most domains are covered — but `Laboratory` and `Physiotherapy` are empty, and a row routed to either gets no reference. `Brand Identity` also keeps its images one level down in `Png/`, and the loader does not recurse | Check the folder for the domain before judging visual output for that row. This trap used to say every folder was empty; that was true until 2026-07-30 and stale afterwards. |
