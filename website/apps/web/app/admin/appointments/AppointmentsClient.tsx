...
import FormField, { selectCls, textareaCls } from '@/components/admin/ui/FormField';
...
export default function AppointmentsClient() {
  ...
  const columns = [
    ...
    { key: 'actions', header: '', width: '50px', render: (r: any) => (
      <button onClick={(e) => { e.stopPropagation(); setSelected(r); setNewStatus(r.status); setNotes(r.notes ?? ''); }} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition"><Eye size={14} /></button>
    )},
  ];
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4">
        <div className="flex gap-3 mb-4">
          ...
          <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            {STATUSES.map(s => <option key={s} value={s}><StatusBadge status={s} /></option>)}
          </select>
        </div>
