...
export const metadata: Metadata = {
  title: 'المراكز الطبية | منظومة إنسان',
      ...
      <section className="py-12 bg-gray-50 min-h-[60vh]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          {result && result.data.length > 0 ? (
            <>
              <p className="text-sm text-gray-500 mb-6">{result.meta.total} مركز طبي</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {result.data.map(c => <MedicalCenterCard key={c.id} center={c} />)}
              </div>
              <Pagination
            ...
            <EmptyState title="لا توجد مراكز طبية حالياً" icon="🏨" />
          )}
        </div>
      </section>
    </PublicLayout>
