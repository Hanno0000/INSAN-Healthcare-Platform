...
docker exec website-api-1 npx ts-node prisma/seed.ts 2>/dev/null || \
docker exec website_api_1 npx ts-node prisma/seed.ts 2>/dev/null || \
docker exec \$(docker ps --filter 'name=api' -q | head -1) npx ts-node prisma/seed.ts

echo '✅ Done!'
"@

$remoteScript = $commands -replace '"', '\"'
