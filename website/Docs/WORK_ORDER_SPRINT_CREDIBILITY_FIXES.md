...
> **للمنفّذ:** الملف ده فيه مهام مرقّمة. نفّذهم بالترتيب المكتوب. كل
...
> **⚠️ تحديث من صاحب المشروع — المهمة 3 (إخفاء الطبيبين والرأي والخبر
> الوهميين) مؤجَّلة. تخطّاها تمامًا ولا تنفّذ أي سطر فيها.** نفّذ المهام
> 1، 2، 4، 5، 6 بس.
...
# ⏸️ المهمة 3 — مؤجَّلة، لا تنفّذها

**سيبها كما هي دلوقتي. الأقسام تحت دي محفوظة للسجل بس — متنفّذش أي سطر منها.**
...
# المهمة 6 — إضافة سؤالين شائعين حقيقيين

صاحب المشروع جاوب على سؤال، ولقيت إجابة سؤال تاني موثّقة بالفعل في ملفات
المشروع (`business/knowledge/departments/MEDICAL_DEPARTMENT_ICU.md` — قسم
"Financial Transparency Framework"، وده نفس المصدر اللي ملف استراتيجية
الأسئلة الشائعة نفسه بيحيل عليه لسؤال التكلفة بالظبط). **السؤال التالت
(المستندات المطلوبة عند الحجز) لسه مالوش إجابة موثّقة — مش جزء من المهمة دي.**

```javascript
// ⛔ ممنوع تعديل أي نص هنا
const FAQ_ITEMS = [
  {
    topic: { ar: 'الإقامة والزيارة', en: 'Staying & Visiting' },
    question: { ar: 'هل ينفع حد من أهلي يبات معايا في المستشفى؟', en: 'Can a family member stay overnight with me in the hospital?' },
    answer: {
      ar: 'في الغرف الداخلية، مسموح لمرافق واحد بالمبيت مع المريض. لو حاسس إنك محتاج شخص معاك طول الوقت لأي سبب، قوله للممرضة المسؤولة وهي هتساعدك تنظّم الموضوع.',
      en: "In inpatient rooms, one companion is allowed to stay overnight with the patient. If you feel you need someone with you for any reason, tell the nurse in charge and they'll help arrange it."
    },
    order: 1,
  },
  {
    topic: { ar: 'التكلفة والتأمين', en: 'Cost & Insurance' },
    question: { ar: 'إمتى هتتكلموا معايا في موضوع التكلفة؟', en: 'When will you talk to me about the cost?' },
    answer: {
      ar: 'من وقت الدخول أو بعده بوقت قصير. هنوضحلك إيه اللي داخل في الخدمة، وإيه اللي ممكن يكون له تكلفة إضافية، وإيه اللي التأمين أو التعاقد بيغطيه — من الأول، مش بعد العلاج. لو عندك أي سؤال مالي في أي وقت، اسأل المسؤول المالي المتاح لك.',
      en: "As early as possible — ideally at admission or shortly after. We'll explain what's included, what may involve extra cost, and what insurance or your contract covers, from the start rather than after treatment. If you have a financial question at any point, ask the financial coordinator assigned to you."
    },
    order: 2,
  },
];
...
for (const item of FAQ_ITEMS) {
  const r = await fetch('/api/v1/admin/faqs', { method: 'POST', headers: H, body: JSON.stringify(item) });
  console.log(item.question.ar, r.status === 201 || r.status === 200 ? '✅' : '❌ ' + r.status + ' ' + await r.text());
}
```

**✅ التحقّق:**
```javascript
const f = (await (await fetch('/api/v1/faqs?pageSize=50')).json()).data;
console.log('عدد الأسئلة الظاهرة على الموقع:', f.length, '(المتوقع: 2)');
f.forEach(x => console.log(' -', x.question?.ar));
```

**وافتح بعينك:** `http://169.58.77.61/faq` — لازم تشوف السؤالين ظاهرين تحت
موضوعيهم.
...
2. **متحطش أي سؤال شائع غير اللي في المهمة 6** — الملف المصدري نفسه بيمنع اختلاق إجابات صراحة. السؤالين في المهمة 6 بس هم اللي عندهم مصدر موثّق.
...
5. **متنفّذش المهمة 3 خالص** — مؤجَّلة، اتقفلت داخل تعليق HTML عمدًا.
...
المهمة 3: ⏸️ مؤجَّلة — لم تُنفَّذ (بالتصميم)
المهمة 4 (تعبئة 6 مراكز): ✅/❌ — [الصق التحقّق]
المهمة 6 (سؤالين شائعين): ✅/❌ — [الصق التحقّق]
